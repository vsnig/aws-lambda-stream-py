...
def initialize(pipelines, **opt):
  ...
  the_pipelines = reduce(lambda acc,id: {
      **acc,
...
def assemble(**opt):
  def assemble_(head, include_fault_handler=True) -> Observable:
    ...
    if len(lines) > 0:
        ...
        def multicast_mapper_(shared): # shared is multicast source Observable
          def pl_mapper(pipeline):
            ...
            return shared.pipe(
              ...
              ops.flat_map(
                lambda x: rx.of(x).pipe(*pipe_ops))
            )
          return rx.merge( 
            *map(pl_mapper, lines)
          )
      ...
      merged = head.pipe(
        ops.multicast(
            subject_factory=lambda _: Subject(), 
            mapper=multicast_mapper(lines)
          ),
      )
      replay = ReplaySubject() #through ReplaySubject otherwise problems with concat on hot observable
